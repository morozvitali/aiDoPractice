package stream.task60x24.x1.task20_collectingAndThen_11;
...
public class Main3 {
    ...
    public String practice9 () {
        return Stream.of(1, 2, 3, 4, 5).filter(a -> a % 2 == 1)
                .map(String::valueOf).collect(Collectors.collectingAndThen(
                        Collectors.toList(),
                        list -> String.join(", ", list)));
    }

    public Integer practice10 () {
        return Stream.of("hi", "hello", "world", "no", "yes").collect(Collectors.collectingAndThen(Collectors.groupingBy(String::length), (Map<Integer, List<String>> map)-> map.size()));
    }
