...
public class Main28 {
    ...
    public Map<String, Long> practice3 () {
        String[] words = {"sky", "apple",
                "moon", "dry", "banana"};
        return Arrays.stream(words).filter(w->w.length() > 3).filter(w->getCount(w)>1).collect(Collectors.toMap(Function.identity(), Main28::getCount));
    }
    ...
    public Map <Character, Long> practice7 () {
        String[] words = {"apple", "orange",
                "banana", "umbrella"};
        return Arrays.stream(words).flatMap(w->w.chars().mapToObj(c->(char)c).filter(c->"aeiou".indexOf(c)>=0))
    ...
    public IntSummaryStatistics practice8 () {
        String[] words = {"apple", "moon",
                "sky", "banana", "loop"};
        return Arrays.stream(words).filter(w-> w.length() - w.chars().distinct().count() > 1).mapToInt(String::length)
                .summaryStatistics();
    }
    ...
    public List <String> practice9 () {
        ...
        return Arrays.stream(words).filter(w->"aeiuo".chars().allMatch(c->w.indexOf(c)>=0)).toList();
    }
