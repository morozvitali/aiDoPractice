...
public class Main7 {
    ...
    public String practice8 () {
        return Stream.of("apple", "Tree",
                "Java", "stream1", "Engineer").filter(w-> Character.isUpperCase(w.charAt(0))).collect(Collectors.toMap(Function.identity(), String::length)).entrySet().stream().max(Comparator.comparing(Map.Entry::getValue)).map(Map.Entry::getKey).orElse("");
    }
}
