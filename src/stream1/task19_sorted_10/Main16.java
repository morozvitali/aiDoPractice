...
public class Main16 {
    ...
    public List <Integer> practice5 () {
        ...
        return nums.stream().sorted(Comparator.comparing(n->(int)n%3).thenComparing(n->(int)n))
                .toList();
    }
    ...
    public List <String> practice7 () {
        ...
        return words.stream().sorted(Comparator.comparing(w->w.chars().filter(c->"aeuio".indexOf(c) >=0).count())).toList();
    }
...
public List<Integer> practice9 () {
    ...
    return numbers.stream().sorted(Comparator.comparing(a -> (int) a % 2).reversed()).toList();}
