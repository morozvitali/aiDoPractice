...
Character mostCommon = words.stream1()
...
List<String> sorted = words.stream1()
...
💡 Підказка: .flatMap(phrase -> Arrays.stream1(phrase.split(" ")))

✅ Рішення:

List<String> unique = phrases.stream1()
.flatMap(p -> Arrays.stream1(p.split(" ")))
...
Map<Integer, List<String>> grouped = words.stream1()
...
💡 Підказка: .flatMap(Arrays::stream1) тут не треба — просто .split(" ").

✅ Рішення:

double avg = Arrays.stream1(text.split(" "))
.mapToInt(String::length)
.average()
.orElse(0);
System.out.println(avg);
